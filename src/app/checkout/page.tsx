...
import { usePurchaseHistory } from "src/contexts/PurchaseHistoryContext";
...
const Checkout = () => {
  ...
  const { cart, clearCart, getCartTotal, removeFromCart, updateQuantity } =
    useCart();
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    const purchaseOrder = {
      ...
      items: cart.map((item) => ({
        ...
        rarity: String(item.card.rarity),
      })),
      subtotal,
      shipping,
      tax,
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description:
          "Your cards are on the way and have been added to your collection!",
      });
            ...
            <div className="card-glass p-6 h-fit space-y-6">
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {cart.map((item) => (
                  <div
                    key={item.card.id}
                    className="flex gap-3 border border-border rounded p-3"
                  >
                    <Image
                      src={item.card.image}
                      alt={item.card.name}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <Button
                          ...
                          className="h-6 w-6"
                          onClick={() =>
                            updateQuantity(item.card.id, item.quantity - 1)
                          }
                        >
                          -
                        </Button>
